package com.czsm.toms.m_hoteltv.interfacesimpl;
...
public class MyMultKeyTrigger implements IMultKeyTrigger {
    ...
    private  boolean checkKeyValid(int num,int delayed){
        ...
        if(check_num< APP.MULT_KEY.length&&APP.MULT_KEY[check_num]==num){
            check_num++;
            return true;
        }else{
...
//      return check_num==MULT_KEY_RESTART.length;
    ...
    public boolean checkMultKey() {
        // TODO Auto-generated method stub
        return check_num == APP.MULT_KEY.length;
    }
    ...
    public void onTrigger() {
        // TODO Auto-generated method stub
        if(checkMultKey()){
        Log.w("the result end","can do it");
        }else{
